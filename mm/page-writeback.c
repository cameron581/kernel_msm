...
#ifdef CONFIG_DYNAMIC_PAGE_WRITEBACK
static void dirty_writeback_suspend(struct early_suspend *h)
{
            printk("[dynamic_writebacks] suspend\n");
                if (dyn_dirty_writeback_enabled)
                        set_dirty_writeback_status(true);
}

static void dirty_writeback_resume(struct early_suspend *h) 
{
            printk("[dynamic_writebacks] resume\n");
                if (dyn_dirty_writeback_enabled)
                    set_dirty_writeback_status(false);
}

static struct early_suspend dirty_writeback_early_suspend_handler = {
	.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN,
	.suspend = dirty_writeback_suspend,
	.resume = dirty_writeback_resume,
};
#endif
/*
...
void __init page_writeback_init(void)
{
  ...
  /* Register the dirty page writeback management during suspend/resume */
	register_early_suspend(&dirty_writeback_early_suspend_handler);
#endif
	writeback_set_ratelimit();
